...
export function useVoiceRoom(roomId: string, session: Session | null, active: boolean) {
  ...
  const toggleMic = useCallback(async () => {
    if (!session) { setError("Canlı konuşmak için Google veya Apple ile giriş yapmalısın."); return; }
    if (muted) {
      try {
      ...
      peersRef.current.forEach((peer) => peer.getSenders().forEach((sender) => { if (sender.track) peer.removeTrack(sender); }));
      setMuted(true);
