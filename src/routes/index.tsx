  ...
  return (
    <main style={{ background:"#07000f", minHeight:"100dvh", fontFamily:"'Helvetica Neue',Arial,sans-serif", position:"relative" }}>
      ...
      <div className="p-scroll">
        ...
        {/* VIP ROZET */}
        {(() => {
          const userVip = 17; // istifadəçinin VIP səviyyəsi
          const displayVip = Math.min(userVip, 15); // max 15 göstər
          return (
            <div style={{
              position:"relative", width:200, height:58, overflow:"hidden",
              borderRadius:10, flexShrink:0
            }}>
              {/* Arxa plan — viparxaplan.png */}
              <img
                src="/images/images/viparxaplan.PNG"
                style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover", borderRadius:10 }}
                alt=""
              />
              {/* Overlay qaraldıcı */}
              <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.3)", borderRadius:10 }}/>
              {/* Sol VIP logo */}
              <img
                src={`/images/images/VIP${displayVip}.png`}
                style={{ position:"absolute", left:6, top:"50%", transform:"translateY(-50%)", width:44, height:44, objectFit:"contain" }}
                alt={`VIP${displayVip}`}
              />
              {/* VIP yazısı */}
              <div style={{
                ...
                <span style={{
                  fontSize:9, fontWeight:600, color:"rgba(200,200,200,.7)",
                  letterSpacing:2, textTransform:"uppercase", fontFamily:"-apple-system,sans-serif"
                }}>Səviyyə</span>
                <span style={{
