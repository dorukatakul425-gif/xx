...
type Member = { user_id: string; role: string; is_muted: boolean; display_name?: string; avatar_url?: string };
...
function AureliaApp() {
  ...
  // Gerçek üye listesi + giriş animasyonu broadcast
  useEffect(() => {
    ...
    // Giriş bildirimi gönder (Supabase Realtime broadcast)
...
function Lobby({ name, onEnter, onBack }: { name: string; onEnter: () => void; onBack: () => void | Promise<void> }) {
  return <main className="room-atmosphere relative min-h-dvh overflow-hidden px-5 text-foreground"><Motes /><div className="relative z-10 mx-auto max-w-md py-6">
    <div className="flex items-center justify-between"><Brand /><Button size="icon" aria-label="Çıkış yap" onClick={onBack}><LogOut className="size-4" /></Button></div>
...
function Room({ name, session, memberCount, members, muted, myEntrance, globalEntrance, onToggleMic, onReplay, onLeave, error }: {
  ...
  onToggleMic: () => void; onReplay: () => void; onLeave: () => void; error: string;
}) {
  // Gerçek konuşmacılar (muted olmayan üyeler)
  ...
  return <main className="room-atmosphere relative min-h-dvh overflow-hidden text-foreground"><Motes /><div className="relative z-10 mx-auto max-w-md px-5 pb-28 pt-5">
    <div className="flex items-center gap-3">
      ...
      <Button size="icon" className="ml-auto" aria-label="Tekrar oynat" onClick={onReplay}><MoreHorizontal className="size-5" /></Button>
    </div>
    ...
    <section className="mt-6 rounded-lg border border-border bg-surface p-4 backdrop-blur-xl">
      <div className="mb-5 flex items-center justify-between"><p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Konuşmacı koltukları</p><span className="text-[10px] text-muted-foreground">{speakers.length} / 6</span></div>
      <div className="grid grid-cols-3 gap-x-3 gap-y-6">
        {speakers.map((m) => (
          <RealSpeaker key={m.user_id} userId={m.user_id} session={session} active={!m.is_muted} />
        ))}
        {Array.from({ length: Math.max(0, 3 - speakers.length) }).map((_, i) => (
          <div key={i} className="flex flex-col items-center"><div className="grid size-16 place-items-center rounded-full border border-dashed border-border text-xl text-muted-foreground">+</div><p className="mt-2 text-xs text-muted-foreground">Boş</p></div>
        ))}
      </div>
    </section>
    ...
    {listeners.length > 0 && (
      <section className="mt-5">
        ...
        <div className="mt-3 flex gap-4 overflow-x-auto pb-1">
          {listeners.slice(0, 8).map((m) => (
            <RealListener key={m.user_id} userId={m.user_id} session={session} />
          ))}
        </div>
      </section>
    )}
  ...
  <div className="fixed inset-x-0 bottom-0 z-20 mx-auto max-w-md bg-background/90 px-5 pb-[max(18px,env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl">
    <div className="flex items-center justify-between rounded-lg border border-border bg-surface p-3">
      <Button size="icon" aria-label={muted ? "Mikrofonu aç" : "Mikrofonu kapat"} onClick={onToggleMic}>{muted ? <MicOff className="size-5" /> : <VoiceIcon />}</Button>
      <Button size="icon" variant="gold" aria-label="Konuş" onClick={onToggleMic}><Mic className="size-6" /></Button>
      <Button size="icon" aria-label="Üyeler"><Users className="size-5" /></Button>
      <Button size="icon" aria-label="Mesaj yaz"><MessageCircle className="size-5" /></Button>
      <Button size="icon" variant="danger" aria-label="Odadan çık" onClick={onLeave}><X className="size-5" /></Button>
    </div>
  </div>

  {/* Kendi giriş animasyonu */}
  {myEntrance && <MyVipEntrance name={name} />}

  {/* Başkasının giriş animasyonu - TikTok tarzı */}
  {globalEntrance && <OtherUserEntrance name={globalEntrance.name} />}
  </main>;
}

// Kendi VIP giriş animasyonu (tam ekran)
...
// Başkasının girişi - TikTok tarzı alttan yukarı banner
function OtherUserEntrance({ name }: { name: string }) {
  return (
    <div
      className="fixed bottom-32 inset-x-0 z-40 mx-auto max-w-md px-5"
      style={{ animation: "slideUpFade 4s ease forwards" }}
    >
      <div className="flex items-center gap-3 rounded-2xl border border-primary/40 bg-background/90 px-4 py-3 shadow-luxe backdrop-blur-xl">
        <div className="grid size-10 shrink-0 place-items-center rounded-full bg-primary text-primary-foreground text-xs font-bold shadow-luxe">
...
// Gerçek kullanıcı konuşmacı kartı
function RealSpeaker({ userId, session, active }: { userId: string; session: Session | null; active: boolean }) {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative">
        {active && <span className="halo absolute inset-0 rounded-full border border-primary/70" />}
        {avatarUrl ? (
          <img src={avatarUrl} alt={label} className="size-16 rounded-full border-2 border-primary/50 object-cover" />
        ) : (
          <div className="size-16 rounded-full border-2 border-primary/50 bg-primary/20 grid place-items-center text-lg font-bold text-primary">
            {initials}
          </div>
        )}
...
function RealListener({ userId, session }: { userId: string; session: Session | null }) {
  ...
  return (
    <div className="shrink-0 text-center">
      {avatarUrl ? (
        <img src={avatarUrl} alt={label} className="size-11 rounded-full border-2 border-primary/50 object-cover" />
      ) : (
        <div className="size-11 rounded-full border-2 border-primary/50 bg-primary/20 grid place-items-center text-sm font-bold text-primary">
          {initials}
        </div>
      )}
