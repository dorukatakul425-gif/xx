...
/* ─── PROFILE ─── */
const AZ_CITIES = ["Bakı","Gəncə","Sumqayıt","Mingəçevir","Naxçıvan","Lənkəran","Şirvan","Yevlax","Xankəndi","Şuşa","Ağdam","Bərdə","Goranboy","Göyçay","İmişli","Kürdəmir","Masallı","Saatlı","Salyan","Sabirabad","Şamaxı","Şəki","Şəmkir","Zaqatala","Balakən","Qax","Quba","Qusar","Lerik","Astara","Cəlilabad","Füzuli","Hadrut","Xocavənd","Laçın","Kəlbəcər","Ağcabədi","Ağdaş","Ağstafa","Ağsu","Biləsuvar","Daşkəsən","Gədəbəy","Gobustan","Hacıqabul","Xızı","Xocaəsgər","İsmayıllı","Lerik","Neftçala","Oğuz","Qazax","Qəbələ","Samux","Şahbuz","Şərur","Terter","Tovuz","Ucar"];
...
function ProfileScreen({ name, onBack, onEnterRoom, onVip }: { name: string; onBack: () => void; onEnterRoom: () => void; onVip: () => void }) {
  ...
  const [profileData, setProfileData] = useState(() => {
    try {
      ...
      return s ? JSON.parse(s) : { username: name || "Doruk A.", gender: "", age: 24, country: "Azərbaycan 🇦🇿", city: "Bakı", bio: "🎮 Oyun sevəni · Gecə quşu 🦅" };
    } catch { return { username: name || "Doruk A.", gender: "", age: 24, country: "Azərbaycan 🇦🇿", city: "Bakı", bio: "🎮 Oyun sevəni · Gecə quşu 🦅" }; }
  });
  ...
  return (
    <main style={{ background:"#07000f", minHeight:"100dvh", fontFamily:"'Helvetica Neue',Arial,sans-serif", position:"relative" }}>
      ...
      <div className="p-scroll">
        <div className="p-hero">
          ...
          <div className="p-top">
            ...
            <button className="p-ibtn" onClick={() => { setDraft(profileData); setEditOpen(true); }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.5)" strokeWidth="2" strokeLinecap="round"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg></button>
          </div>
          <div className="p-body">
            ...
            <div style={{ flex:1, paddingBottom:2 }}>
              <div style={{ fontSize:21, fontWeight:900, color:"#fff", marginBottom:3 }}>{profileData.username}</div>
              <div style={{ fontSize:11, color:"rgba(255,255,255,.35)", marginBottom:10, lineHeight:1.4 }}>{profileData.bio}</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:10 }}>
                {[
                  {txt: profileData.country},
                  {txt: `${profileData.age} yaş`},
                  {txt: profileData.city},
                ].map((m,i) => (
        ...
        {(() => {
          ...
          return (
            <div style={{
              position:"relative", width:210, height:60, overflow:"hidden",
              borderRadius:10, flexShrink:0, margin:"12px 18px 0"
            }}>
              ...
              <img src={`/images/images/VIP${displayVip}.png`}
                style={{ position:"absolute", left:8, top:"50%", transform:"translateY(-50%)", width:44, height:44, objectFit:"contain" }}
                alt={`VIP${displayVip}`}/>
              {/* VIP yazısı — sol tərəf, qızıl */}
              <div style={{
                ...
                <span style={{
                  ...
                  background:"linear-gradient(135deg,#fff8c0,#ffd700,#c8860a)",
      ...
      {/* EDIT MODAL */}
      {editOpen && (
        <div style={{ position:"fixed", inset:0, zIndex:999, background:"rgba(0,0,0,.85)", backdropFilter:"blur(8px)", overflowY:"auto" }}>
          <div style={{ maxWidth:430, margin:"0 auto", padding:"max(24px,env(safe-area-inset-top)) 0 40px" }}>
            {/* Header */}
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 20px 20px" }}>
              <button onClick={() => setEditOpen(false)} style={{ background:"rgba(255,255,255,.08)", border:"1px solid rgba(255,255,255,.1)", borderRadius:12, width:38, height:38, color:"#fff", fontSize:18, cursor:"pointer" }}>✕</button>
              <span style={{ fontSize:16, fontWeight:700, color:"#fff" }}>Profili Düzəlt</span>
              <button onClick={() => { saveProfile(draft); setEditOpen(false); }} style={{ background:"linear-gradient(135deg,#7b2ff7,#ff3ea5)", border:"none", borderRadius:12, padding:"8px 16px", color:"#fff", fontSize:13, fontWeight:700, cursor:"pointer" }}>Saxla</button>
            </div>

            {/* Şəkil */}
            <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)", borderRadius:18, margin:"0 18px 10px", padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <span style={{ fontSize:13, color:"rgba(255,255,255,.5)" }}>Şəkil</span>
              <div onClick={() => document.getElementById("edit-avatar-input")?.click()} style={{ width:48, height:48, borderRadius:"50%", background:"conic-gradient(#ffd700,#c084fc,#ffd700)", padding:2, cursor:"pointer" }}>
                <div style={{ width:"100%", height:"100%", borderRadius:"50%", background:"#1a0035", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, fontWeight:900, color:"#fff" }}>
            ...
            {/* İstifadəçi adı */}
            <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)", borderRadius:18, margin:"0 18px 10px", padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
              <span style={{ fontSize:13, color:"rgba(255,255,255,.5)", flexShrink:0 }}>İstifadəçi adı</span>
              <input value={draft.username} onChange={e => setDraft({...draft, username:e.target.value})}
                style={{ background:"transparent", border:"none", outline:"none", color:"#fff", fontSize:14, fontWeight:600, textAlign:"right", flex:1 }}
                placeholder="Ad daxil edin"/>
            </div>

            {/* Cins */}
            <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)", borderRadius:18, margin:"0 18px 10px", padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
              <span style={{ fontSize:13, color:"rgba(255,255,255,.5)", flexShrink:0 }}>Cins</span>
              <div style={{ display:"flex", gap:8 }}>
                {["Kişi","Qadın","Digər"].map(g => (
                  <button key={g} onClick={() => setDraft({...draft, gender:g})}
                    style={{ padding:"6px 12px", borderRadius:10, border:`1px solid ${draft.gender===g ? "#7b2ff7" : "rgba(255,255,255,.1)"}`, background:draft.gender===g ? "rgba(123,47,247,.3)" : "transparent", color:draft.gender===g ? "#c084fc" : "rgba(255,255,255,.4)", fontSize:12, cursor:"pointer" }}>
                    {g}
                  </button>
                ))}
              </div>
            ...
            {/* Yaş */}
            <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)", borderRadius:18, margin:"0 18px 10px", padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
              <span style={{ fontSize:13, color:"rgba(255,255,255,.5)", flexShrink:0 }}>Yaş</span>
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                <button onClick={() => setDraft({...draft, age:Math.max(13,draft.age-1)})} style={{ width:32, height:32, borderRadius:10, border:"1px solid rgba(255,255,255,.1)", background:"rgba(255,255,255,.05)", color:"#fff", fontSize:18, cursor:"pointer" }}>−</button>
                <span style={{ fontSize:18, fontWeight:700, color:"#fff", minWidth:28, textAlign:"center" }}>{draft.age}</span>
                <button onClick={() => setDraft({...draft, age:Math.min(99,draft.age+1)})} style={{ width:32, height:32, borderRadius:10, border:"1px solid rgba(255,255,255,.1)", background:"rgba(255,255,255,.05)", color:"#fff", fontSize:18, cursor:"pointer" }}>+</button>
              </div>
            </div>

            {/* Ölkə */}
            <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)", borderRadius:18, margin:"0 18px 10px", padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
              <span style={{ fontSize:13, color:"rgba(255,255,255,.5)", flexShrink:0 }}>🌍 Ölkə</span>
              <select value={draft.country} onChange={e => setDraft({...draft, country:e.target.value})}
                style={{ background:"transparent", border:"none", outline:"none", color:"#fff", fontSize:13, fontWeight:600, textAlign:"right", flex:1, cursor:"pointer" }}>
            ...
            {/* Bölgə */}
            <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)", borderRadius:18, margin:"0 18px 10px", padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
              <span style={{ fontSize:13, color:"rgba(255,255,255,.5)", flexShrink:0 }}>Yaşadığın bölgə</span>
              <select value={draft.city} onChange={e => setDraft({...draft, city:e.target.value})}
                style={{ background:"transparent", border:"none", outline:"none", color:"#fff", fontSize:13, fontWeight:600, textAlign:"right", flex:1, cursor:"pointer" }}>
                {AZ_CITIES.map(c => <option key={c} value={c} style={{ background:"#1a0035" }}>{c}</option>)}
              </select>
            </div>

            {/* Haqqında */}
            <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)", borderRadius:18, margin:"0 18px 10px", padding:"16px 20px" }}>
              <span style={{ fontSize:13, color:"rgba(255,255,255,.5)", display:"block", marginBottom:10 }}>Haqqında</span>
              <textarea value={draft.bio} onChange={e => setDraft({...draft, bio:e.target.value})}
                rows={3} placeholder="Özün haqqında yaz..."
                style={{ width:"100%", background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.08)", borderRadius:12, padding:"10px 12px", color:"#fff", fontSize:13, resize:"none", outline:"none", fontFamily:"inherit" }}/>
            </div>
          </div>
        </div>
      )}
