...
function Room({ name, session, memberCount, members, muted, myEntrance, globalEntrance, onToggleMic, onJoinSeat, onLeaveSeat, onReplay, onLeave, error }: {
  ...
  onToggleMic: () => void; onJoinSeat: () => void; onLeaveSeat: () => void; onReplay: () => void; onLeave: () => void; error: string;
}) {
  ...
  return <main className="room-atmosphere relative min-h-dvh overflow-hidden text-foreground"><Motes /><div className="relative z-10 mx-auto max-w-md px-5 pb-28 pt-5">
    ...
    <section className="mt-6 rounded-lg border border-border bg-surface p-4 backdrop-blur-xl">
      ...
      <div className="grid grid-cols-3 gap-x-3 gap-y-6">
        {speakers.map((m) => (
          <RealSpeaker key={m.user_id} userId={m.user_id} session={session} speaking={!m.is_muted} onLeave={m.user_id === session?.user.id ? onLeaveSeat : undefined} />
        ))}
        {Array.from({ length: emptySeatCount }).map((_, i) => (
          <button key={i} onClick={iAmSpeaker ? undefined : onJoinSeat} disabled={iAmSpeaker} className="flex flex-col items-center group">
            <div className={`grid size-16 place-items-center rounded-full border-2 border-dashed transition-all text-2xl ${iAmSpeaker ? "border-border text-muted-foreground" : "border-primary/50 text-primary/70 group-active:bg-primary/10 group-hover:border-primary group-hover:text-primary"}`}>+</div>
            <p className={`mt-2 text-xs ${iAmSpeaker ? "text-muted-foreground" : "text-primary/80"}`}>{iAmSpeaker ? "Dolu" : "Katıl"}</p>
          </button>
        ))}
      </div>
    </section>
  ...
  return (
    <div className="fixed bottom-32 inset-x-0 z-40 mx-auto max-w-md px-5" style={{ animation: "slideUpFade 4s ease forwards" }}>
      <div className="flex items-center gap-3 rounded-2xl border border-primary/40 bg-background/90 px-4 py-3 shadow-luxe backdrop-blur-xl">
        ...
        <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{name}</p><p className="text-[10px] text-primary/80 uppercase tracking-wider">odaya katıldı · VIP 17</p></div>
...
function RealSpeaker({ userId, session, speaking, onLeave }: { userId: string; session: Session | null; speaking: boolean; onLeave?: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative" onClick={onLeave}>
        {speaking && (
          <>
        ...
        {avatarUrl
          ...
          : <div className="size-16 rounded-full border-2 border-primary/50 bg-primary/20 grid place-items-center text-lg font-bold text-primary">{initials}</div>
        }
        ...
        {isMe && <span className="absolute -top-1 -right-1 grid size-5 place-items-center rounded-full bg-red-500 text-white text-[8px] font-bold">✕</span>}
      </div>
