...
function AureliaApp() {
  ...
  const signIn = async (provider: "google" | "apple") => {
    ...
    if (signInError) setError("Giriş başlatılamadı. Lütfen tekrar deneyin.");
    setLoading(null);
  };
...
function Room({
  ...
  return (
    <main className="room-atmosphere relative min-h-dvh overflow-hidden text-foreground">
      <Motes />
      <div className="relative z-10 mx-auto flex min-h-dvh max-w-md flex-col px-4 pb-[max(16px,env(safe-area-inset-bottom))] pt-4">
        ...
        <section className="mt-5 rounded-lg border border-border bg-surface p-4 backdrop-blur-xl">
          ...
          <div className="grid grid-cols-3 gap-x-3 gap-y-5">
            ...
            {Array.from({ length: emptySeatCount }).map((_, index) => (
              <Button
                ...
                disabled={iAmSpeaker}
                className="h-auto flex-col gap-2 py-1"
                aria-label={
                  iAmSpeaker
              ...
              >
                <span className="grid size-16 place-items-center rounded-full border-2 border-dashed border-primary/45 text-xl text-primary">
                  +
                </span>
                <span className="text-[10px] text-muted-foreground">
        ...
        <section className="mt-4 min-h-16">
          <div className="flex items-center justify-between">
            <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
              Dinleyiciler
            </p>
        ...
        <footer className="sticky bottom-0 mt-3 bg-background/90 pb-[max(4px,env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl">
          <div className="flex items-center justify-between rounded-lg border border-border bg-surface p-3 shadow-luxe">
...
function ChatDock({
  ...
  return (
    <section className="mt-4 flex min-h-64 flex-1 flex-col overflow-hidden rounded-lg border border-border bg-surface/80 backdrop-blur-xl">
      ...
      <div
        className="max-h-64 min-h-40 flex-1 space-y-3 overflow-y-auto px-4 py-3"
        aria-live="polite"
      >
