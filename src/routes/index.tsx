...
function ProfileScreen({ name, onBack, onEnterRoom, onVip }: { name: string; onBack: () => void; onEnterRoom: () => void; onVip: () => void }) {
  ...
  const [profileData, setProfileData] = useState(() => {
    try {
      ...
      return s ? JSON.parse(s) : { username: name || "İstifadəçi", gender: "", age: 18, country: "Azərbaycan", city: "Bakı", bio: "" };
  ...
  // Saxta ziyarətçilər (demo)
  const visitors = [
    { name:"Aynur M.", time:"2 dəq əvvəl", avatar:"A", vip:7, country:"Azərbaycan" },
    { name:"Rauf K.", time:"15 dəq əvvəl", avatar:"R", vip:3, country:"Türkiyə" },
    { name:"Sevinc H.", time:"1 saat əvvəl", avatar:"S", vip:12, country:"Azərbaycan" },
    { name:"Tural B.", time:"3 saat əvvəl", avatar:"T", vip:0, country:"Rusiya" },
    { name:"Nigar A.", time:"Dünən 22:14", avatar:"N", vip:5, country:"Azərbaycan" },
  ];
  return (
    <main style={{ background:"#07000f", minHeight:"100dvh", fontFamily:"'Helvetica Neue',Arial,sans-serif", position:"relative" }}>
      ...
      <div className="p-scroll">
        <div className="p-hero">
          ...
          <div className="p-body">
            <div style={{ position:"relative", flexShrink:0 }}>
              <div className="p-av-outer">
                <div id="p-main-avatar" className="p-av-inner"
                  ref={el => {
                    if (el) { try { const av = localStorage.getItem("profile_avatar"); if (av) el.innerHTML = `<img src="${av}" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>`; else el.textContent = (profileData.username[0] || "İ").toUpperCase(); } catch {} }
                  }}>
            ...
            <div style={{ flex:1, paddingBottom:2 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:6 }}>
                <span style={{ fontSize:11, color:"rgba(255,255,255,.3)", fontWeight:500 }}>ID: {userId}</span>
              ...
              {profileData.bio ? <div style={{ fontSize:11, color:"rgba(255,255,255,.35)", marginBottom:8, lineHeight:1.4 }}>{profileData.bio}</div> : null}
              {/* Meta pillər */}
        ...
        {/* VIP ROZET */}
        <div style={{ position:"relative", width:220, height:62, overflow:"hidden", borderRadius:12, margin:"14px 18px 0" }}>
          <img src="/images/images/viparxaplan.PNG" style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover" }} alt=""/>
      ...
      {/* ZİYARƏTÇİLƏR PANELİ */}
      {visitorOpen && (
        <div style={{ position:"fixed", inset:0, zIndex:998, background:"rgba(0,0,0,.85)", backdropFilter:"blur(8px)" }} onClick={() => setVisitorOpen(false)}>
          <div style={{ position:"absolute", bottom:0, left:0, right:0, maxWidth:430, margin:"0 auto", background:"linear-gradient(180deg,#120025,#0a001a)", borderRadius:"24px 24px 0 0", border:"1px solid rgba(255,255,255,.07)", overflow:"hidden" }}
            onClick={e => e.stopPropagation()}>
            {/* Handle */}
            <div style={{ display:"flex", justifyContent:"center", paddingTop:12 }}><div style={{ width:36, height:4, borderRadius:2, background:"rgba(255,255,255,.12)" }}/></div>
            <div style={{ padding:"16px 20px 0", display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
              ...
              <div style={{ textAlign:"right" }}>
                <div style={{ fontSize:24, fontWeight:900, color:"#c084fc" }}>143</div>
                <div style={{ fontSize:10, color:"rgba(192,132,252,.5)" }}>ziyarət</div>
              </div>
            </div>
            {/* Ziyarətçi siyahısı */}
            <div style={{ padding:"0 20px 32px", display:"flex", flexDirection:"column", gap:10 }}>
              {visitors.map((v, i) => (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.05)", borderRadius:16 }}>
                  {/* Avatar */}
                  <div style={{ width:42, height:42, borderRadius:"50%", background:`conic-gradient(#7b2ff7,#ff3ea5,#7b2ff7)`, padding:2, flexShrink:0 }}>
                    <div style={{ width:"100%", height:"100%", borderRadius:"50%", background:"#1a0035", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, fontWeight:800, color:"#fff" }}>{v.avatar}</div>
                  </div>
                  ...
                  <div style={{ flexShrink:0 }}>
                    {v.vip > 0
                      ? <div style={{ background:"rgba(255,200,0,.1)", border:"1px solid rgba(255,200,0,.2)", borderRadius:8, padding:"3px 8px", fontSize:10, fontWeight:700, color:"#ffd700" }}>VIP{v.vip}</div>
                      : <div style={{ background:"rgba(255,255,255,.05)", border:"1px solid rgba(255,255,255,.08)", borderRadius:8, padding:"3px 8px", fontSize:10, color:"rgba(255,255,255,.25)" }}>VIP0</div>
                    }
                  </div>
                </div>
              ))}
              {/* VIP deyilsə blur overlay */}
              {displayVip === 0 && (
                <div style={{ position:"relative", borderRadius:16, overflow:"hidden" }}>
                  <div style={{ padding:"12px 14px", background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.05)", borderRadius:16, filter:"blur(4px)", pointerEvents:"none" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                      <div style={{ width:42, height:42, borderRadius:"50%", background:"rgba(255,255,255,.1)" }}/>
                      <div><div style={{ width:80, height:12, borderRadius:6, background:"rgba(255,255,255,.1)", marginBottom:6 }}/><div style={{ width:50, height:9, borderRadius:6, background:"rgba(255,255,255,.07)" }}/></div>
                    </div>
                  </div>
                  <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"rgba(10,0,25,.7)", borderRadius:16 }}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,200,0,.6)" strokeWidth="2" strokeLinecap="round" style={{ marginBottom:6 }}><path d="M2 8l4 8h12l4-8-5 3-5-7-5 7-5-3z"/></svg>
                    <span style={{ fontSize:11, color:"rgba(255,200,0,.7)", fontWeight:600 }}>VIP1+ lazımdır</span>
                  </div>
                </div>
              )}
            </div>
              ...
              <input id="edit-av-inp" type="file" accept="image/*" style={{ display:"none" }}
                onChange={e => {
                  ...
                  reader.onload = ev => {
                    ...
                    // Avatar ana
                    const mainAv = document.getElementById("p-main-avatar");
                    if (mainAv) { mainAv.innerHTML = `<img src="${b64}" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>`; }
