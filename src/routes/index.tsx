...
function VelvetApp() {
  ...
  const [maintenance, setMaintenance] = useState(() => {
    try { return localStorage.getItem("velvet_maintenance") === "1"; } catch { return false; }
  });
  ...
  // Admin paneldən maintenance dəyişikliyini 2 saniyədə bir yoxla
  useEffect(() => {
    const poll = setInterval(() => {
      try {
        const m = localStorage.getItem("velvet_maintenance") === "1";
        setMaintenance(m);
        // VIP dəyişikliyini yoxla
        const savedVip = localStorage.getItem("velvet_admin_vip");
        if (savedVip !== null) {
          const newVip = parseInt(savedVip);
          if (!isNaN(newVip)) {
            try {
              const p = localStorage.getItem("velvet_profile");
              const parsed = p ? JSON.parse(p) : {};
              parsed.vip = newVip;
              localStorage.setItem("velvet_profile", JSON.stringify(parsed));
            } catch {}
            localStorage.removeItem("velvet_admin_vip");
          }
        }
      } catch {}
    }, 2000);
  ...
  if (splash) return (
    <>
      <style>{GLOBAL_CSS}{`
        ...
        .splash-wrap{position:fixed;inset:0;z-index:9999;background:#07000f;display:flex;flex-direction:column;align-items:center;justify-content:center;animation:splashFade .4s ease}
        .splash-logo{animation:logoFloat 3s ease-in-out infinite}
        .splash-bar-track{width:200px;height:4px;background:rgba(255,255,255,.06);border-radius:2px;overflow:hidden;margin-top:48px}
        ...
        .splash-text{font-size:11px;color:rgba(255,255,255,.3);letter-spacing:3px;margin-top:16px;text-transform:uppercase}
  ...
  if (maintenance) return <><style>{GLOBAL_CSS}</style><MaintenanceScreen onAdminUnlock={() => { setMaintenance(false); try { localStorage.removeItem("velvet_maintenance"); } catch {} }} /></>;
...
/* ─── MAINTENANCE SCREEN ─── */
function MaintenanceScreen({ onAdminUnlock }: { onAdminUnlock: () => void }) {
  ...
  const tapBrand = () => {
    const n = adminTap + 1;
    setAdminTap(n);
    if (n >= 7) { setAdminOpen(true); setAdminTap(0); }
  };
  const tryAdmin = () => {
    if (adminPass === "Velvet2025!") { onAdminUnlock(); setAdminOpen(false); }
    else { setAdminErr("Şifrə yanlışdır"); setTimeout(() => setAdminErr(""), 2000); }
  };
  return (
    <main style={{ position:"fixed", inset:0, background:"#07000f", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden", fontFamily:"'Helvetica Neue',Arial,sans-serif" }}>
      <style>{`
        @keyframes mscan{0%{top:-100%}100%{top:200%}}
        @keyframes morbit{from{transform:translate(-50%,-50%) rotate(0deg)}to{transform:translate(-50%,-50%) rotate(360deg)}}
        @keyframes mpulse{0%,100%{opacity:.3;transform:translate(-50%,-50%) scale(1)}50%{opacity:.6;transform:translate(-50%,-50%) scale(1.07)}}
        @keyframes micon{0%,100%{filter:drop-shadow(0 0 12px rgba(123,47,247,.4))}50%{filter:drop-shadow(0 0 28px rgba(123,47,247,.8)) drop-shadow(0 0 50px rgba(255,62,165,.3))}}
        @keyframes mbar{0%{background-position:-200% 0}100%{background-position:200% 0}}
        @keyframes mfloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}}
        @keyframes mparticle{0%{transform:translateY(0) scale(1);opacity:1}100%{transform:translateY(-140px) scale(0);opacity:0}}
        @keyframes mfade{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
        @keyframes mwarn{0%,100%{opacity:.6}50%{opacity:1}}
      `}</style>

      {/* Grid arxa plan */}
      <div style={{ position:"absolute", inset:0, backgroundImage:"linear-gradient(rgba(123,47,247,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(123,47,247,.04) 1px,transparent 1px)", backgroundSize:"44px 44px" }}/>

      {/* Orb-lar */}
      <div style={{ position:"absolute", width:600, height:600, borderRadius:"50%", background:"radial-gradient(circle,rgba(123,47,247,.12) 0%,transparent 70%)", top:"50%", left:"50%", transform:"translate(-50%,-50%)", animation:"mpulse 7s ease-in-out infinite" }}/>
      <div style={{ position:"absolute", width:400, height:400, borderRadius:"50%", background:"radial-gradient(circle,rgba(255,62,165,.08) 0%,transparent 70%)", top:"50%", left:"50%", transform:"translate(-50%,-50%)", animation:"mpulse 6s ease-in-out infinite .5s" }}/>

      {/* Fırlanan halqalar */}
      {[600,450,300].map((s,i) => (
        <div key={i} style={{ position:"absolute", width:s, height:s, borderRadius:"50%", border:`1px solid rgba(${i===1?"255,62,165":"123,47,247"},.0${i===0?5:i===1?4:6})`, top:"50%", left:"50%", transform:"translate(-50%,-50%)", animation:`morbit ${35-i*8}s linear infinite ${i%2===1?"reverse":""}` }}/>
      ))}

      {/* Scan xətti */}
      <div style={{ position:"absolute", left:0, right:0, height:2, background:"linear-gradient(90deg,transparent,rgba(123,47,247,.4),rgba(255,62,165,.3),transparent)", animation:"mscan 4s linear infinite", boxShadow:"0 0 20px rgba(123,47,247,.4)" }}/>

      {/* Hissəciklər */}
      {[
        {l:"10%",c:"#7b2ff7",d:"0s",tx:"18px"},{l:"25%",c:"#c084fc",d:".7s",tx:"-12px"},
        {l:"45%",c:"#ff3ea5",d:"1.3s",tx:"8px"},{l:"60%",c:"#7b2ff7",d:".3s",tx:"-20px"},
        {l:"75%",c:"#c084fc",d:"1.8s",tx:"14px"},{l:"88%",c:"#ff3ea5",d:".9s",tx:"-8px"},
      ].map((p,i) => (
        <div key={i} style={{ position:"absolute", bottom:"15%", left:p.l, width:5, height:5, borderRadius:"50%", background:p.c, animation:`mparticle 4s ease-out ${p.d} infinite`, boxShadow:`0 0 8px ${p.c}` }}/>
      ))}

      {/* Məzmun */}
      <div style={{ position:"relative", zIndex:2, display:"flex", flexDirection:"column", alignItems:"center", padding:"24px", textAlign:"center", maxWidth:400, animation:"mfade .6s ease" }}>

        {/* Animasyonlu ikon */}
        <div style={{ width:96, height:96, borderRadius:26, background:"rgba(123,47,247,.1)", border:"1px solid rgba(123,47,247,.2)", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:28, position:"relative", animation:"micon 3s ease-in-out infinite" }}>
          <div style={{ position:"absolute", inset:-10, borderRadius:34, border:"1px solid rgba(123,47,247,.07)", animation:"mpulse 3s ease-in-out infinite .3s" }}/>
          <div style={{ position:"absolute", inset:-20, borderRadius:42, border:"1px solid rgba(123,47,247,.04)", animation:"mpulse 3s ease-in-out infinite .6s" }}/>
          <svg width="46" height="46" viewBox="0 0 24 24" fill="none">
            <defs>
              <linearGradient id="mig1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#7b2ff7"/><stop offset="100%" stopColor="#c084fc"/></linearGradient>
              <linearGradient id="mig2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#c084fc"/><stop offset="100%" stopColor="#ff3ea5"/></linearGradient>
            </defs>
            <path d="M14.7 6.3a1 1 0 000 1.4l1.6 1.6a1 1 0 001.4 0l3.77-3.77a6 6 0 01-7.94 7.94l-6.91 6.91a2.12 2.12 0 01-3-3l6.91-6.91a6 6 0 017.94-7.94l-3.76 3.76z" stroke="url(#mig1)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
            <circle cx="12" cy="12" r="2" fill="url(#mig2)" opacity=".5"/>
          </svg>
        </div>

        {/* Brand — 7x bas admin giriş üçün */}
        <div style={{ fontSize:10, letterSpacing:5, color:"rgba(255,255,255,.2)", textTransform:"uppercase", marginBottom:10, cursor:"default" }} onClick={tapBrand}>VELVET</div>
        <div style={{ fontSize:28, fontWeight:900, letterSpacing:-.5, marginBottom:8, background:"linear-gradient(135deg,#fff,#c084fc,#ff3ea5)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Texniki Xidmət</div>
        <div style={{ fontSize:14, color:"rgba(255,255,255,.45)", lineHeight:1.8, marginBottom:32, maxWidth:320 }}>
          Tətbiqimizi sizin üçün daha yaxşı etmək üzərindəyik. Qısa müddətdə geri qayıdacağıq.
        </div>

        {/* Progress bar */}
        <div style={{ width:260, marginBottom:28 }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
            <span style={{ fontSize:9, letterSpacing:2, color:"rgba(255,255,255,.2)", textTransform:"uppercase" }}>İrəliləyiş</span>
            <span style={{ fontSize:9, letterSpacing:2, color:"rgba(255,255,255,.2)" }}>68%</span>
          </div>
          <div style={{ height:3, background:"rgba(255,255,255,.06)", borderRadius:2, overflow:"hidden" }}>
            <div style={{ width:"68%", height:"100%", background:"linear-gradient(90deg,#7b2ff7,#c084fc,#ff3ea5)", backgroundSize:"200% 100%", animation:"mbar 2s linear infinite", borderRadius:2 }}/>
          </div>
        </div>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", justifyContent:"center", marginBottom:32 }}>
          {[
            {dot:"#50c050",glow:"rgba(80,200,80,.6)",txt:"Server aktiv"},
            {dot:"#ff9500",glow:"rgba(255,150,0,.6)",txt:"Yeniləmə gedir",blink:true},
            {dot:"#50c050",glow:"rgba(80,200,80,.6)",txt:"Məlumat qorunur"},
          ].map((p,i) => (
            <div key={i} style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.07)", borderRadius:20, padding:"7px 13px" }}>
              <div style={{ width:6, height:6, borderRadius:"50%", background:p.dot, boxShadow:`0 0 6px ${p.glow}`, animation:p.blink?"mwarn 2s ease-in-out infinite":undefined }}/>
              <span style={{ fontSize:11, color:"rgba(255,255,255,.4)" }}>{p.txt}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ fontSize:12, color:"rgba(255,255,255,.2)", letterSpacing:1 }}>
          Təxmini bitiş: <span style={{ color:"#c084fc", fontWeight:700, fontSize:15 }}>{fmt(secs)}</span>
        </div>
      </div>

      {/* Admin giriş modal — 7x tapa açılır */}
      {adminOpen && (
        <div style={{ position:"fixed", inset:0, zIndex:999, background:"rgba(0,0,0,.9)", backdropFilter:"blur(10px)", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"linear-gradient(135deg,#120025,#0a001a)", border:"1px solid rgba(255,255,255,.08)", borderRadius:22, padding:28, width:"100%", maxWidth:320, margin:20, animation:"mfade .3s ease" }}>
            <div style={{ fontSize:15, fontWeight:700, color:"#fff", marginBottom:6 }}>Admin Girişi</div>
            <div style={{ fontSize:11, color:"rgba(255,255,255,.3)", marginBottom:20 }}>Tətbiqi yenidən açmaq üçün şifrənizi daxil edin</div>
            <input value={adminPass} onChange={e => setAdminPass(e.target.value)}
              onKeyDown={e => e.key === "Enter" && tryAdmin()}
              type="password" placeholder="Admin şifrəsi"
              style={{ width:"100%", background:"rgba(255,255,255,.05)", border:"1px solid rgba(255,255,255,.1)", borderRadius:12, padding:"12px 14px", color:"#fff", fontSize:14, outline:"none", marginBottom:8 }}/>
            {adminErr && <div style={{ color:"#ff3ea5", fontSize:11, marginBottom:8 }}>{adminErr}</div>}
            <div style={{ display:"flex", gap:8, marginTop:4 }}>
              <button onClick={() => { setAdminOpen(false); setAdminPass(""); }} style={{ flex:1, background:"rgba(255,255,255,.05)", border:"1px solid rgba(255,255,255,.08)", borderRadius:12, padding:12, color:"rgba(255,255,255,.4)", fontSize:13, cursor:"pointer" }}>Ləğv et</button>
              <button onClick={tryAdmin} style={{ flex:1, background:"linear-gradient(135deg,#7b2ff7,#ff3ea5)", border:"none", borderRadius:12, padding:12, color:"#fff", fontSize:13, fontWeight:700, cursor:"pointer" }}>Daxil ol</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <main style={{ background:"#07000f", minHeight:"100dvh", fontFamily:"'Helvetica Neue',Arial,sans-serif", position:"relative" }}>
      ...
      <div className="p-scroll">
        ...
        {/* VIP ROZET + ID — Premium animasyonlu */}
        <style>{`
          @keyframes vipShimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}
          @keyframes vipGlow{0%,100%{box-shadow:0 0 12px rgba(255,200,0,.2)}50%{box-shadow:0 0 28px rgba(255,200,0,.5),0 0 50px rgba(255,150,0,.2)}}
          @keyframes idPulse{0%,100%{opacity:.7}50%{opacity:1}}
          @keyframes vipBadgePop{0%{transform:scale(1)}50%{transform:scale(1.04)}100%{transform:scale(1)}}
        `}</style>
        <div style={{ margin:"12px 18px 0", display:"flex", alignItems:"center", gap:10 }}>
          {/* VIP rozet — daha dar, sağdan kəsilmiş */}
          <div style={{ position:"relative", width:148, height:54, overflow:"hidden", borderRadius:12, animation:"vipGlow 3s ease-in-out infinite", flexShrink:0 }}>
            ...
            <div style={{ position:"absolute", left:54, top:"50%", transform:"translateY(-50%)" }}>
              <span style={{
                fontSize:20, fontWeight:900, letterSpacing:.5, lineHeight:1,
                background:"linear-gradient(135deg,#fff8c0 0%,#ffd700 40%,#ff9500 70%,#ffd700 100%)",
          ...
          {/* ID kartı — VIP17 xüsusi premium, digərləri normal */}
          {userVip === 17 ? (
            <div style={{ flex:1, height:54, borderRadius:12, position:"relative", overflow:"hidden" }}>
              {/* VIP17 xüsusi arxa plan */}
              <div style={{ position:"absolute", inset:0, background:"linear-gradient(135deg,#1a0035,#2d0060,#1a0035)" }}/>
              <div style={{ position:"absolute", inset:0, background:"linear-gradient(90deg,rgba(255,180,0,.08),rgba(255,100,0,.05),rgba(255,180,0,.08))", backgroundSize:"200% 100%", animation:"vipShimmer 2s linear infinite" }}/>
              {/* Üst-alt parlama xətti */}
              <div style={{ position:"absolute", top:0, left:0, right:0, height:1, background:"linear-gradient(90deg,transparent,rgba(255,200,0,.5),transparent)" }}/>
              <div style={{ position:"absolute", bottom:0, left:0, right:0, height:1, background:"linear-gradient(90deg,transparent,rgba(255,150,0,.3),transparent)" }}/>
              <div style={{ position:"relative", zIndex:2, height:"100%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:1 }}>
                <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                  <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#ffd700" strokeWidth="2"><path d="M12 2l3 6 7 1-5 5 1 7-6-3-6 3 1-7-5-5 7-1z"/></svg>
                  <span style={{ fontSize:8, letterSpacing:3, color:"rgba(255,200,0,.6)", textTransform:"uppercase" }}>PREMIUM ID</span>
                  <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#ffd700" strokeWidth="2"><path d="M12 2l3 6 7 1-5 5 1 7-6-3-6 3 1-7-5-5 7-1z"/></svg>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                  <span style={{ fontSize:17, fontWeight:900, letterSpacing:2, background:"linear-gradient(135deg,#fff8c0,#ffd700,#ff9500,#ffd700,#fff8c0)", backgroundSize:"300% 100%", animation:"vipShimmer 1.8s linear infinite", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", filter:"drop-shadow(0 0 8px rgba(255,200,0,.4))" }}>{userId}</span>
                  <button onClick={() => { navigator.clipboard?.writeText(userId).then(() => { setCopyDone(true); setTimeout(() => setCopyDone(false), 1500); }); }}
                    ...
                    {copyDone
                      ? <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#ffd700" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
                      : <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(255,200,0,.4)" strokeWidth="2" strokeLinecap="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
                    }
                  </button>
                </div>
              </div>
            ...
            <div style={{ flex:1, height:54, borderRadius:12, position:"relative", overflow:"hidden", background:"linear-gradient(135deg,#0e0025,#1a003a)", border:"1px solid rgba(192,132,252,.2)", animation:"vipBadgePop 4s ease-in-out infinite" }}>
              <div style={{ position:"absolute", inset:0, background:"linear-gradient(90deg,transparent,rgba(192,132,252,.05),transparent)", backgroundSize:"200% 100%", animation:"vipShimmer 3s linear infinite" }}/>
              <div style={{ position:"relative", zIndex:2, height:"100%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:2 }}>
                <span style={{ fontSize:9, letterSpacing:3, color:"rgba(192,132,252,.4)", textTransform:"uppercase" }}>İstifadəçi ID</span>
                <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                  <span style={{ fontSize:15, fontWeight:800, letterSpacing:1, background:"linear-gradient(135deg,#c084fc,#ff3ea5,#c084fc)", backgroundSize:"200% 100%", animation:"vipShimmer 2.5s linear infinite", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>{userId}</span>
                  <button onClick={() => { navigator.clipboard?.writeText(userId).then(() => { setCopyDone(true); setTimeout(() => setCopyDone(false), 1500); }); }}
                    style={{ background:"transparent", border:"none", cursor:"pointer", padding:2, display:"flex" }}>
                    {copyDone
                      ...
                      : <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(192,132,252,.4)" strokeWidth="2" strokeLinecap="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
                    }
                  </button>
                </div>
              </div>
        ...
        <div className="p-menu">
          ...
          {[
            ...
            { png:"magaza.PNG", label:"Mağaza", onClick: undefined },
            ...
            <div key={i} className="p-mi" onClick={item.onClick} style={item.onClick ? { cursor:"pointer" } : {}}>
              <div className="p-mi-l" style={{ background:"rgba(255,255,255,.04)", overflow:"hidden" }}>
